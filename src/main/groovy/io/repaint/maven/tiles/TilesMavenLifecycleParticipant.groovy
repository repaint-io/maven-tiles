...
public class TilesMavenLifecycleParticipant extends AbstractMavenLifecycleParticipant {
	...
	protected void orchestrateMerge(MavenProject project, boolean allowSnapshots) throws MavenExecutionException {
		...
		loadAllDiscoveredTiles(project,allowSnapshots)
...
//				new org.apache.maven.model.building.ModelData(new FileModelSource(tileModel.tilePom), model));
	...
	protected void loadAllDiscoveredTiles(MavenProject project, boolean allowSnapshots) throws MavenExecutionException {
		...
		resolvePluginsVersion(project,allowSnapshots)
	 ...
	 */
	void resolvePluginsVersion(MavenProject project, boolean allowSnapshots) {
		versionnedPlugins.keySet().each { String key ->
			...
			if(plugins) {
				...
				plugins.each { Plugin plugin ->
					mavenVersionIsolate.resolvePluginVersionRange(plugin,versionRange,allowSnapshots)
				}
			}
		}
	}
