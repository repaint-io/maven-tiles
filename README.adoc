= Tiles Maven Plugin - Version 2.5
...
- Your tile cannot be part of a reactor build, it must exist before the reactor build takes place, because that
  process only ever occurs once and the parent build order is otherwise unknown. If you find issues in your reactor
  build like this, please let us know on the https://github.com/repaint-io/maven-tiles/issues/[Github issues].
- We do not support issues raised where you are using a multi-module build. These are the
  anti-thesis of Repaint.IO's philosophy of composition over inheritance. If you raise the issue and it seems a
  reasonable use case we will look at it, but please be aware that it is unlikely to be prioritised.
